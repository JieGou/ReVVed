...
Category("Behavior"),
DefaultValue(false),
Description("Controls whether the listview should strip RTF formatting")
]
        public bool StripRTF
        {
            get
            {
                return strip_rtf;
            ...
            if (DataMemberChanged != null)
      {
        ...
        private ListViewItem BuildItemForRow(object row)
        {
            ...
            for (int column = 0; column < itemText.Length; ++column)
            {
                ...
                //strips RTF formatting
                string s = m_properties[column].GetValue(row).ToString();

                if (strip_rtf && s.Length >= 6 && s.Substring(0, 6) == "{\\rtf1")
                {
      ...
      if (!m_changingIndex)
        ...
        //the function below is required if the listview should process the delete key
