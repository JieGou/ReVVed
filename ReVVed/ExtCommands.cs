...
namespace RVVD
{
  ...
  public class open_folder : IExternalCommand
  {
    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
      ...
      try
      {
        ...
        if (directory != null && directory != string.Empty && !File.Exists(directory))
        {
        ...
        if (directory != null && directory != string.Empty)
        {
          ...
          myProcess.StartInfo.FileName = Path.GetDirectoryName(directory);
  ...
  public class options : IExternalCommand
  {
    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
      ...
      try
      {
        IList<RibbonPanel> panels = commandData.Application.GetRibbonPanels();
        int x = 0;
        RibbonPanel rp = null;
        ...
        IList<RibbonItem> r_items = rp.GetItems();
  ...
  public class revved_about : IExternalCommand
  {
    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
      try
      {
        ...
        ab.WindowStartupLocation = System.Windows.WindowStartupLocation.CenterOwner;
        ab.ShowDialog();
